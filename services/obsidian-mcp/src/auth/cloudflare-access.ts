// Cloudflare Access AuthProvider.
//
// Cloudflare adds the `Cf-Access-Jwt-Assertion` header to requests that
// pass an Access policy. The JWT is RS256-signed using a key fetched from
// the team's JWKS endpoint:
//
//   https://<team>.cloudflareaccess.com/cdn-cgi/access/certs
//
// Verification covers:
//   - Signature against the JWKS (with kid lookup, not a cached key).
//   - `iss` matches `https://<team>.cloudflareaccess.com`.
//   - `aud` matches the AUD tag of the Access application.
//   - Expiry / nbf are honoured by jose.
//   - `email` claim is present and used as the Identity.
//
// Bearer-token defence-in-depth runs alongside this — see ./bearer.ts and
// the wiring in main.ts.
//
// Future migrations: implement a sibling AuthProvider (IAPAuthProvider,
// OIDCAuthProvider) and swap the layer in main.ts. The boundary upstream
// of this module is just `validateRequest: AuthRequest -> Effect<Identity>`.
// Read docs/obsidian-mcp/auth.md for the full migration recipe.
...
import { type AuthProviderImpl, AuthProvider, type AuthRequest } from "./provider.js";
...
const identityFrom = (payload: CfClaims, source: string): Effect.Effect<Identity, AuthError> => {
  const email = payload.email ?? (payload.common_name ? `service-token:${payload.common_name}` : undefined);
  if (!email) {
    return Effect.fail(
      new AuthError({
...
const make = (cfg: CloudflareAccessConfig): AuthProviderImpl => {
  // jose's createRemoteJWKSet caches the JWKS for 10 minutes by default and
  // re-fetches on cache miss + on key rotation (kid mismatch). That's
  // exactly the cache behaviour we want — no manual cache layer needed.
...
export const CloudflareAccessAuthProviderLayer = (cfg: CloudflareAccessConfig) =>
  Layer.succeed(AuthProvider, make(cfg));
