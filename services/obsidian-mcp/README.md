...
- `src/couchdb/` — `CouchClient/` + `CouchClientLayer/`, `Vault/` + `VaultLayer/`, `subscribeChanges/` for the `_changes` feed, `syncParameters/` for bootstrapping the master PBKDF2 salt at startup, and the LiveSync E2EE primitives (`encryptField/`, `decryptField/`, `encryptMeta/`, `decryptMeta/`, `path2id/`, `splitIntoChunks/`, `chunkId/`, `assembleChunks/`).
