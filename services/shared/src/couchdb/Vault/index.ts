...
export interface VaultImpl {
  ...
  >;
  /**
   * Resolve a CouchDB document _id (not a path) to a fully decrypted
   * NoteRead. Used by the vault-indexer's changes-feed pipeline, which
   * receives doc ids from the CouchDB `_changes` stream and needs both
   * the canonical vault path AND the body in one round-trip.
   *
   * Fails with NoteNotFoundError when the id does not point at a live
   * note doc (tombstoned, system doc, chunk doc, or simply missing).
   */
  readonly readNoteById: (
    id: string,
  ) => Effect.Effect<
    NoteRead,
    CouchDbError | DecryptionError | NoteNotFoundError | EncryptionError
