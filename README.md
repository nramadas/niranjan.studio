...
- **Obsidian MCP server** (Phase 2): a Cloud Run service that exposes the same vault to Claude over the Model Context Protocol (presented to Claude as **Sutra** — the service, package, and infra keep the `obsidian-mcp` name; only the connector's advertised identity is branded). Reads and writes encrypted notes through the LiveSync E2EE format. Reached at `mcp.<domain>` via Cloud Run's native domain mapping (no Cloudflare in this path); gated by an OAuth 2.1 server inside the service that uses Google as the OIDC identity provider. See [docs/obsidian-mcp/setup.md](docs/obsidian-mcp/setup.md).
...
- **Google Meet transcript ingestion + digest** (Phase 5): zero-touch ingestion of Google Meet's own transcripts, from **multiple Google accounts** (personal + work both feed the one vault) — per-account Workspace Events subscriptions (owned and auto-renewed by the MCP service itself) publish `transcript.v2.fileGenerated` events to a shared Pub/Sub topic that pushes to `mcp.<domain>/meet/webhook`; the service resolves which account can read the meeting, pulls the transcript via the Meet REST API, and writes it into the vault as an encrypted note. A Claude-powered digest then merges action items into a single `TODO.md` (urgent items first, deduplicated) and accumulates per-person dossiers under `People/`. See [docs/obsidian-mcp/meet-ingestion.md](docs/obsidian-mcp/meet-ingestion.md).
...
│   ├── meet-ingestion.tf             Phase 5: Meet/Events/Pub-Sub APIs, meet-events topic + push subscription, accounts-JSON + Anthropic secrets.
