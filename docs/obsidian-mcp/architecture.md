...
The MCP server speaks LiveSync's CouchDB document format directly. Three shapes matter:

- **Note documents** (`type: "newnote" | "plain"`, `_id` prefixed `f:` when path obfuscation is on). When the plugin has *Obfuscate Properties* enabled, the doc-level `mtime`/`ctime`/`size`/`children` fields are deliberately zeroed and the real values live inside an encrypted JSON blob in the `path` field, prefixed `/\:`. The server reads this blob via [decryptMeta/](../../services/obsidian-mcp/src/couchdb/decryptMeta/) and writes it via [encryptMeta/](../../services/obsidian-mcp/src/couchdb/encryptMeta/); every read site is funnelled through a single `toLogicalDoc` helper so doc-level fields are never accidentally trusted.
- **Chunk documents** (`type: "leaf"`). `_id` is prefixed `h:` for plaintext chunks and `h:+` for encrypted chunks — the `+` is load-bearing because the plugin's `isEncryptedChunkEntry` gate keys on it; without it our ciphertext gets pasted into notes as literal text. Encrypted chunks also carry an `e_: true` marker.
- **The `_local/obsidian_livesync_sync_parameters` doc.** Holds the master PBKDF2 salt the plugin uses for HKDF, base64-encoded. The server reads this at boot via [syncParameters/](../../services/obsidian-mcp/src/couchdb/syncParameters/) and refuses to start if it's absent — better to fail loudly than to silently encrypt with a wrong salt.

Encryption is delegated to `octagonal-wheels` (the same npm library the LiveSync plugin uses). Each LiveSync primitive lives in its own function-folder under [services/obsidian-mcp/src/couchdb/](../../services/obsidian-mcp/src/couchdb/) — `decryptField/`, `encryptField/`, `decryptMeta/`, `encryptMeta/`, `path2id/`, `splitIntoChunks/`, `chunkId/`, `assembleChunks/`, `syncParameters/`. Format-specific constants (`PREFIX_CHUNK = "h:"`, `PREFIX_ENCRYPTED_CHUNK = "h:+"`, `PREFIX_OBFUSCATED = "f:"`, `ENCRYPTED_META_PREFIX = "/\:"`, and the four encryption-prefix variants `%$`/`%=`/`%~`/`%`) live in `src/couchdb/constants.ts`. Primitives and constants are faithful to the LiveSync source as of the version pinned in [troubleshooting.md](troubleshooting.md).

For maximum compatibility, the server writes in the V2 HKDF fixed-salt format with `Obfuscate Properties` semantics (chunks with `%=` data + `h:+` `_id` + `e_: true`, note doc with encrypted meta blob in `path`). Reads handle `%=`, `%$`, `%~`, and `%` transparently. `path2id` runs case-insensitive by default to match the plugin's `!handleFilenameCaseSensitive` default — vault paths are matched case-insensitively just like the plugin does. Writes don't dedupe chunks the way the plugin does (it uses content-defined chunking with locality awareness and xxhash64 IDs vs. our SHA-256); the next plugin-side normalisation pass converges on the plugin's preferred chunk shape, so dedup is approximate until then.

If the LiveSync plugin's chunk format changes in a future release, reads of existing notes may start failing with `DecryptionError` or returning blank bodies. The recovery is to bump the `octagonal-wheels` dependency and update the relevant primitive folder. See troubleshooting.md for the recipe.
