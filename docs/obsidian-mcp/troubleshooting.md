...
| Some reads return `DecryptionError`, others succeed | Mixed encryption formats in the same vault. The server emits HKDF fixed-salt (`%=`, the V2 plugin default) and reads HKDF fixed (`%=`), HKDF ephemeral (`%$`), V3 (`%~`), and V2 (`%`). If one specific note fails, the chunk's format may have changed mid-write. Open the note in Obsidian and re-save to force re-encryption in the current format. |
| `CouchDbError { op: "readSyncParameters" }` at boot | The `_local/obsidian_livesync_sync_parameters` doc doesn't exist on the database yet. The LiveSync plugin writes it during its first sync — connect a plugin client to the database before starting the server, or trigger a manual sync to seed the doc. |
| Plugin log shows `Entry has been obfuscated!` after a write from MCP | The meta blob written by `create_note`/`update_note` had its inner `path` field prefixed with `f:`. The plugin assigns it back to `doc.path` after decrypt and `id2path_base` recursively trips the obfuscated guard. The server now stores the *plain* vault path inside the meta blob; if you still see this, you're on an older revision of the server — redeploy and re-run the failing write to overwrite the bad doc. |
| Plugin log shows `File X seems to be corrupted! Writing prevented. (a != b)` | Size-field mismatch. The plugin compares the byte length of the reassembled body against the `size` in the meta blob. If they differ, it refuses to apply the file locally. Cause is usually multi-byte UTF-8 content (em-dashes, curly quotes, emoji) where the server used UTF-16 char length instead of byte length. Server now uses `Buffer.byteLength` everywhere — if you still see this, re-run the failing `create_note`/`update_note` to overwrite the bad doc with one that carries the correct byte size. |
| Plugin shows raw `%=…` ciphertext pasted inline into a note | Chunk `_id`s lack the `h:+` prefix the plugin's `isEncryptedChunkEntry` gate requires. Without it the plugin treats the chunk's `data` as plaintext and inserts the ciphertext literally. Re-running the write produces chunks with correct IDs; the bad chunks become orphan junk in CouchDB (safe to leave; cleanup query in [services/obsidian-mcp/src/couchdb/](../../services/obsidian-mcp/src/couchdb/)). |
...
The server's chunking + path-obfuscation + encryption logic (split across [services/obsidian-mcp/src/couchdb/path2id/](../../services/obsidian-mcp/src/couchdb/path2id/), [splitIntoChunks/](../../services/obsidian-mcp/src/couchdb/splitIntoChunks/), [chunkId/](../../services/obsidian-mcp/src/couchdb/chunkId/), [decryptField/](../../services/obsidian-mcp/src/couchdb/decryptField/), [encryptField/](../../services/obsidian-mcp/src/couchdb/encryptField/), [decryptMeta/](../../services/obsidian-mcp/src/couchdb/decryptMeta/), [encryptMeta/](../../services/obsidian-mcp/src/couchdb/encryptMeta/), [syncParameters/](../../services/obsidian-mcp/src/couchdb/syncParameters/), with format constants in [constants.ts](../../services/obsidian-mcp/src/couchdb/constants.ts)) is faithful to LiveSync as of:
